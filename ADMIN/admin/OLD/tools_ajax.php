...
if (!empty($f['material_id'])) {
    ...
    foreach ($tables as $t) {
        ...
        $where[] = "(U.tbl = ? AND U.tool_id IN (
                        SELECT tool_id FROM $tm WHERE material_id IN ($ph)
                     ))";
        $bind[] = $t;
        foreach ($f['material_id'] as $m) $bind[] = $m;
    }
}
...
if (!empty($f['strategy_id'])) {
    ...
    foreach ($tables as $t) {
        ...
        $bind[] = $t;
        $bind[] = $t;
        foreach ($f['strategy_id'] as $s) $bind[] = $s;
    }
}

// 7) Ensamblar SQL final
$sql  = "SELECT * FROM (\n$baseSql\n) AS U";
if ($where) {
    $sql .= "\nWHERE " . implode("\n  AND ", $where);
}
...
foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    ...
    // parámetros
    $mt = 'toolsmaterial_' . substr($row['tbl'],6);
    $p  = $pdo->prepare("
      SELECT m.name, vc_m_min, fz_min_mm, fz_max_mm, ap_slot_mm, ae_slot_mm
        FROM $mt tm
