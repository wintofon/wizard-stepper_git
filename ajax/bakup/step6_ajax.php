...
// 4) mmr (Material Removal Rate)
$mmr = ($ap * $feed * $aeSlot) / 1000;

// 5) Fc (Fuerza de corte)
$Fc = ($Kc11 * pow(max(1e-6, $fz), -$mc) * $ap * $fz * $Z * (1 + $coef * tan($alpha))) / cos($phi);

// 6) Potencia en watts (corrigiendo vc a m/s)
$vc_ms = ($rpm * M_PI * $d) / 60000;  // rpm → m/s
$watts = ($Fc * $vc_ms) / $eta;

// 7) Potencia en HP
$hp = $watts / 745.7;

// Radar chart (valores subjetivos)
