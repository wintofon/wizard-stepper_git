 ...
 */
/** Ubicación: C:\xampp\htdocs\wizard-stepper_git\assets\js\step7.js */
/* global Chart */
...
window.initStep7 = function () {
  ...
  const csrfToken = window.step7Csrf;
  ...
  async function recalc() {
    ...
    table(payload);
    try {
      const res = await fetch(`${BASE_URL}/ajax/step7_ajax_legacy_minimal.php`, {
        method: 'POST',
        headers: {
  ...
  updatePasadasInfo();
  recalc();
  if (!window.step7ErrorHandlerAdded) {
    window.addEventListener('error', ev => showError(`JS: ${ev.message}`));
    window.step7ErrorHandlerAdded = true;
  }
};
