...
(() => {
  ...
  /* ─────────────────── PARAMS & DOM ─────────────────── */
  // Recuperamos los parámetros pasados desde PHP
  const P = window.step6Params;
  if (!P) return alert('⚠️  Parámetros técnicos faltantes.');

  const {
    diameter:    D,                  // Diámetro de la herramienta (mm)
    flute_count: Z,                  // Número de filos
    rpm_min:     RPM_MIN,            // RPM mínimo
    rpm_max:     RPM_MAX,            // RPM máximo
    fr_max:      FR_MAX,             // Feedrate máximo (mm/min)
    coef_seg:    K_SEG_transmission, // Coeficiente de seguridad (transmisión)
    Kc11:        KC,                 // Coeficiente específico de corte
    mc:          MC,                 // Exponente mc
    eta:         ETA,                // Eficiencia
    fz0:         FZ0,                // Avance base por diente (mm)
    vc0:         VC0,                // Velocidad de corte base (m/min)
    thickness:   THK,                // Espesor del material (mm)
    cut_length:  CUT_LEN,            // Longitud de filo (mm)
    hp_avail:    HP_AVAIL,           // Potencia disponible (HP)
    fz_min0:     FZ_MIN,             // fz mínimo base
    fz_max0:     FZ_MAX,             // fz máximo base
    angle_ramp:  ANGLE_RAMP          // Ángulo de rampa (º)
  } = P;
  ...
  // Escalamos los límites de fz según el coeficiente de seguridad
  ...
  /* ───────────────────── STATE ────────────────────── */
  const state = {
    fz: +FZ0 * K_SEG_transmission,  // Avance inicial escalado
    vc: +VC0,                       // Velocidad de corte actual
    ae: D * 0.5,                    // Ancho de pasada inicial
    ap: 1,                          // Número de pasadas inicial
    last: {}                        // Snapshot previo para diff
  };
  ...
  /* ─────────────────── VALIDATIONS ─────────────────── */
  const validateFeed   = vf  => (vf < FR_MAX ? hideAlert(feedAlert) : showAlert(feedAlert, `Feedrate > ${FR_MAX} mm/min`));
  ...
  function beautify(slider, dec) {
    ...
    const paint = v => {
      ...
      // Si está en el extremo, marcar en rojo
  ...
  /**
   * ─────────────────── PASADAS SYNC ───────────────────
   * Ajusta el rango de pasadas para que la profundidad
   * (THK / pasadas) nunca exceda CUT_LEN.
   */
  function syncPass() {
    const minP = Math.max(1, Math.ceil(THK / CUT_LEN));
  ...
  function recalc() {
    ...
    // 2) Ajuste automático de fz si supera feedrate
    if (vfRaw >= FR_MAX) {
      state.fz = FR_MAX / (N * Z);
    ...
    validateRpm(N);
    validateLength();
    // calculamos porcentaje de uso de HP
    ...
    render({
      ...
      fc:      Math.round(fcVal),
      hp:      parseFloat(hpVal.toFixed(1)),
      w:       Math.round(wVal * 1000),
      eta:     Math.min(100, Math.round(powerPct)),
      life:    lifePct,
      power:   powerPct,
      finish:  finishPct,
      fz:      state.fz
  ...
  try {
    // ─ Inicialización de rangos y valores
