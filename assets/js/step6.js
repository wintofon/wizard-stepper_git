 ...
 *             y escalado por coeficiente de seguridad de la transmisión.
 *      – ae  limitado por el diámetro de la herramienta.
 *      – ap  número de pasadas, tal que la profundidad ≤ largo de filo.
 *  • Radar Chart de 3 ejes: Vida Útil · Potencia · Terminación.
 *      – Coloreado verde/naranja/rojo según uso de potencia.
 *  • Alertas:
 *      – Feedrate > máximo.
 *      – RPM fuera de rango.
 *      – Profundidad de material > largo de corte.
 *      – Potencia > 80 % de disponible.
 * ====================================================================*/
(() => {
  'use strict';
  ...
  const TAG   = '[Step6]';
  const say   = (lvl, ...msg) => { if (DEBUG) console[lvl](`${TAG}`, ...msg); };
  const log   = (...msg) => say('log', ...msg);
  const warn  = (...msg) => say('warn', ...msg);
  const error = (...msg) => say('error', ...msg);
  /** detecta cambios en snapshot para no volver a renderizar igual */
  const diff = (a={}, b={}) => {
  ...
  // renombramos coef_seg → K_SEG_transmission y damos default 1.2
  const {
    diameter:    D,
    flute_count: Z,
    rpm_min:     RPM_MIN,
  ...
  // ajustar límites fz por coef. seguridad
  ...
  const hmCalc   = (fz, ae)  => {
    ...
    return phi ? (fz*(1 - Math.cos(phi)) / phi) : fz;
  };
  ...
  /*** PASADAS SYNC: ap ≤ CUT_LEN ***/
  function syncPass() {
    // ≥ pasadas para que ap = THK/p ≤ CUT_LEN
    const minPasses = Math.ceil(THK / CUT_LEN);
    // ≤ pasadas para que ap no sea menor que 1 pasada
