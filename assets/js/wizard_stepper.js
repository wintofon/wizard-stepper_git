      ...
      .then(html => {
        ...
        if (step === 6) {
          if (!window.step6Loaded) {
            ...
            script.src = `${BASE_URL}/assets/js/step6_auto_expert_result.js`;
            script.defer = true;
            script.onload = () => { 
              window.step6Loaded = true;
              log('[stepper.js] 🔢 step6_auto_expert_result.js cargado OK');
              if (typeof window.initStep6 === 'function') window.initStep6();
            };
            script.onerror = () => console.error('[stepper.js] ⚠️ Error cargando step6_auto_expert_result.js');
