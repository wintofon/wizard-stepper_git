...
try {
    ...
    // 5) Datos de la herramienta
    $tool = ToolModel::getTool($pdo, $toolTable, $toolId) ?: [];
    $D        = (float) ($tool['diameter_mm']       ?? 0);
    $Z        = (int)   ($tool['flute_count']       ?? 1);
    $shank    = (float) ($tool['shank_diameter_mm'] ?? 0);
    $fluteLen = (float) ($tool['flute_length_mm']   ?? 0);
    $cutLen   = (float) ($tool['cut_length_mm']     ?? 0);
    $fullLen  = (float) ($tool['full_length_mm']    ?? 0);
