<?php
/**
 * File: views/steps/step7.php
 * Descripción: Paso 7 – Resultados expertos del Wizard CNC
 ...
 *   - window.step7Params y window.step7Csrf para el JS
 */

declare(strict_types=1);

if (!getenv('BASE_URL')) {
    // Sube 3 niveles: /views/steps/step7.php → /wizard-stepper_git
    putenv(
        'BASE_URL=' . rtrim(
...
if ($missing) {
    ...
    echo "<pre class='step7-error'>ERROR – faltan claves en sesión:\n" . implode(', ', $missing) . "</pre>";
    exit;
}
...
foreach ([
    'src/Controller/ExpertResultController_backup.php',
...
$step7JsRel      = asset('assets/js/step7.js');
...
<head>
  ...
  <title>Cutting Data – Paso&nbsp;7</title>
  <?php
    $styles = [
      $cssBootstrapRel,
...
<!-- SCRIPTS -->
<script>window.step7Params = <?= $jsonParams ?>; window.step7Csrf = '<?= $csrfToken ?>';</script>
