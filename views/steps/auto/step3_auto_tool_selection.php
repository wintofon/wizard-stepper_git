...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stepRaw !== 3) {
        ...
        echo "<script>alert('Paso inválido. Reinicia el wizard.'); window.location='step3_auto_tool_selection.php';</script>";
        exit;
    }
    ...
    if (!is_int($toolIdRaw) || $toolIdRaw <= 0) {
        ...
        echo "<script>alert('ID de herramienta inválido.'); window.location='step3_auto_tool_selection.php';</script>";
        exit;
    }
    if (!in_array($toolTblClean, $allowed, true)) {
        dbg("❌ [step3][POST] tool_table inválido: {$toolTblRaw} → limpio: {$toolTblClean}");
        echo "<script>alert('Tabla de herramienta inválida.'); window.location='step3_auto_tool_selection.php';</script>";
        exit;
    }
...
<head>
  ...
  <link rel="stylesheet" href="<?= asset('assets/css/pages/_step3_auto_tool_selection.css') ?>">
