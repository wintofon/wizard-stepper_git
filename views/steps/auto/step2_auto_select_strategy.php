 ...
 * • Chequea que wizard_progress>=1 (sino redirige a step1_auto_select_material.php)
 * • Carga dinámicamente estrategias según tipo
 * • Guarda {machining_type_id, strategy_id} en sesión y avanza a step3_auto_choose_tool.php
 * ---------------------------------------------------------------
 */
...
if (empty($_SESSION['wizard_progress']) || (int)$_SESSION['wizard_progress'] < 1) {
    dbg('❌ wizard_progress<1, redirigiendo a step1_auto_select_material.php');
    header('Location: step1_auto_select_material.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        session_write_close();
        header('Location: step3_auto_choose_tool.php');
        exit;
    }
}
